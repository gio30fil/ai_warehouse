...
# VIEW RAW SOFTONE DATA
@main.route("/admin/softone-raw")
def softone_raw():
    ...
    # Create a lookup for stock (case-insensitive and stripped)
    stock_lookup = {
...
# VIEW ALL STOCK FROM DATABASE
@main.route("/admin/stock")
def admin_stock():
    ...
    live_stock = fetch_stock() # fetches all warehouses

    # Create a lookup for stock breakdown
    # { item_code: { total: X, details: [ {wh: Y, s: Z}, ... ] } }
    stock_lookup = {}
    for s in live_stock:
        code = str(s.get('item_code', '')).strip().lower()
        if not code: continue
        
        # breakdown is in 'stock_per_warehouse'
        details = []
        for wh in s.get('stock_per_warehouse', []):
            wh_name = wh.get('whouse_name', 'Unknown')
            # Warning: Key has a trailing space in the API response: "physical_stock "
    ...
    cursor.execute("""
        SELECT kodikos, factory_code, description, category, subcategory, stock 
        FROM products 
    ...
    # Enrich database products with live breakdown
    enriched_products = []
    for row in products_db:
        code = str(row[0]).strip().lower()
        live = stock_lookup.get(code, {"total": row[5], "details": []})
...
@main.route("/admin/pending-orders")
def pending_orders():
    ...
    # Notice: fetching live documents might be slow or return empty if not found
    try:
        orders = fetch_pending_orders()
        # In SoftOne, sometimes we might need to filter manually for 'pending'
        # but let's assume the service 'getSalesDocuments' is already scoped or returns status.
    except Exception as e:
