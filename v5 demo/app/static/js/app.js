...
function renderProducts(products, container) {
    ...
    products.forEach((p, index) => {
        ...
        const fcodeClean = (p.factory_code || '').replace(/'/g, "\\'");
            ...
            <div class="product-header">
                <div class="product-info">
                    <div class="product-meta">
                        S1 Code: <span class="badge badge-code" style="font-size:0.7rem; padding: 0.1rem 0.35rem; font-family: monospace;">${p.kodikos}</span> 
                        | Factory Code: <strong style="color: var(--text-main);">${p.factory_code || 'N/A'}</strong>
                    </div>
                    <h4>${p.description}</h4>
                ...
                <div class="text-right" style="display: flex; flex-direction: column; align-items: flex-end; gap: 0.35rem;">
                    ...
                    <div style="display: flex; align-items: center; gap: 0.4rem;">
                        ...
                        <button class="btn btn-secondary btn-sm datasheet-btn" 
                                style="padding: 0.25rem 0.6rem; font-size: 0.75rem; font-weight: 500; display: inline-flex; align-items: center; gap: 0.3rem; border-radius: var(--radius-sm);"
                                onclick="openDatasheet('${fcodeClean}', '${descClean}')">
                            📄 Datasheet
                        </button>
                    </div>
                </div>
...
function openDatasheet(factoryCode, description) {
    const searchTerm = (factoryCode && factoryCode !== 'N/A' && factoryCode.trim() !== '') 
        ? factoryCode.trim() 
        : description.trim();
    const query = encodeURIComponent(searchTerm + ' datasheet pdf');
    ...
    products.forEach(p => {
        ...
        row.innerHTML = `
            <td>
                ...
                ${p.factory_code ? `<br><span class="text-xs text-muted" style="font-family: monospace; font-size: 0.75rem; display: inline-block; margin-top: 4px;">Fact: ${p.factory_code}</span>` : ''}
            </td>
    ...
    li.innerHTML = `
        <div class="offer-item-content">
            <div class="offer-item-info">
                <strong class="offer-code" style="color: var(--primary);">${code}</strong> 
                ${factory && factory !== 'None' ? `<span class="text-muted" style="font-size: 0.75rem; font-family: monospace; margin-left: 5px; opacity: 0.85;">(${factory})</span>` : ''} 
